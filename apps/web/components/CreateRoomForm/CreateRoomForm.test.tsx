...
describe("CreateRoomForm", () => {
  ...
  it("creates a room with the default Cast mode, stores the participant token, and navigates to it", async () => {
  ...
  it("shows the Cast option for guests too, since it no longer requires signing in", async () => {
    const form = new CreateRoomFormPageObject()
    expect(form.queryCastModeButton()).toBeInTheDocument()
  })

  it("switches to and sends playlist mode once selected", async () => {
    vi.mocked(api.post).mockResolvedValue({
      room: {
        id: "r1",
        code: "ABC123",
